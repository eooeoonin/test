 ...
 */
@Controller
@RequestMapping("/web_mgt/w_pc_news_list")
public class PcNewsController {

	private final static WinsMoneyLogger logger = WinsMoneyLoggerFactory.getLogger(PcNewsController.class);
	...
	public BannerResForm getNewsById(BannerReqVo bannerReqVo, String bizeCode) {
		...
		String content = htmlParser.parseHtml(filePath+"/pc_" +fileName);
		/*HtmlParser htmlParser = new HtmlParser();
	...
	public Map<String, String> publish(BannerReqVo bannerReqVo, String bizCode, String content, HttpServletRequest httpRequest) {
		...
		try {
			...
			velocityGenerator.buildPcNewsByTemplate(entityObject, fileName, filePath);
			if (result.isSuccess()) {
