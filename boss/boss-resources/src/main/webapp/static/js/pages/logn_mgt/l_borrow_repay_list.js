	  ...
	  $.ajax({
	      ...
	      success: function(responseMsg) {
	    	  ...
	    	  $.each(responseMsg.list, function (k, v) {
		          ...
		          html +="<td>"+v.borrowedAmount.amount+"</td>";
		          html +="<td>"+v.nextPhase+"/"+v.totalPhase+"</td>";
		          html +="<td>"+v.nextReapyAmount.amount+"</td>";
		          if(v.protocolId == null || v.protocolId == ""){
		        	  v.protocolId = "——";
		          }
		          if(v.nextRepayTime != null){
		        	  html +="<td>"+v.nextRepayTime.split(" ")[0]+"</td>";
		          	  html +="<td>"+v.protocolId+"</td>";
		          	  html +="<td><a href='repayList_repayDetail.html?borrowId="+v.id+"&repayTime="+v.nextRepayTime.split(" ")[0]+"'>查看</a></td>";
		          }else{
		        	  html +="<td>"+v.nextRepayTime+"</td>";
		        	  html +="<td>"+v.protocolId+"</td>";
		          	  html +="<td><a href='repayList_repayDetail.html?borrowId="+v.id+"'>查看</a></td>";
		          }
		          html +="</tr>";
	    	  });
	    	  ...
	    	  html +='<td style="text-align:center; vertical-align:middle;" colspan="7">没有找到匹配的记录</td>';
	    	  html += '</tr>';
	    	  $("#table").find("tbody").html(html);
		 	  $("#tcdPagehide").hide();
	      }
