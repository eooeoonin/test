 $(function() {
	...
	var tdUrl = "/web_mgt/w_app_news_list/getNewsById?bizeCode="+html;
	var tbData = {
		"id":GetRequest().id
	};
...
function tableFun(tdUrl, tbData) {
	$.ajax({
		...
		success : function(data) {
			...
			document.getElementById("wxTitle").value = data.wxTitle;
			document.getElementById("wxBody").value = data.wxBody;
			document.getElementById("fileName").value = data.picture;
			document.getElementById("fileWXName").value = data.wxPicture;
...
$("#submitBtn").click(function(){
	if (!_modalFm1.validationEngine('validate')) {
		...
		var url = "/web_mgt/w_app_news_list/publish?bizCode=" + html;
		$.ajax({
			type : "POST",
			url : url,
			dataType : "json",
...
$("#viewHtml").click(function(){
	var id = $("#id").val();
	window.location.href = domainUrl + html+"/a_" + id + ".html";
});
