...
$(function () {
  ...
  $(".fancybox").fancybox({openEffect: "none", closeEffect: "none"});
	...
	$.ajax({
		type : "POST",
		url : "/borrow/financeReview_I_financial_audit/audit",
		...
		success : function(data) {
			if (data != null && data != "") {
				if (data.resCode == 0) {
					...
					if(_data.termType == "MONTH"){
						$("#approveTerm").val(_data.approveTerm);//回显批贷期限
					}else if(_data.termType == "DAY"){
						$("#approveTerm").val(_data.approveTerm);//回显批贷期限
					}else{
						$("#approveTerm").val("——");//回显借款期限
					}
...
$("#addLoan").click(function(){
	...
	$.ajax({
		type : "POST",
		url : "/borrow/financeReview_I_financial_audit/finalReview",
		data : {'id':borrowId,'mark':mark,'reviewCode':reviewCode,'editBy':editBy,'borrowExtendType':'FINANCE'},
		success : function(data) {
			if (data != null && data != "") {
				if (data.resCode == 0) {
					bootbox.alert("操作成功", function(){
						location.href = "/borrow/financeReview_list.html";
					});
				}else{
