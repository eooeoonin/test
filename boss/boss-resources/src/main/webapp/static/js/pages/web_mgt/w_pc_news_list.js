...
$(function() {
	...
	tableFun("/web_mgt/w_pc_news_list/listByFilter",tData);
	myPage();
});
...
$("#srhBtn").click(function() {  
	...
	tableFun("/web_mgt/w_pc_news_list/listByFilter", srhData);
	myPage();
});
	...
	$.ajax({
		...
		success:function(data){
			...
			$.each(data.list,function(k,v){
				...
				tableBodyHtml += '<td style="position:relative"><a href="w_pc_news_list_edit.html?id='+ v.id + '" style="margin-left:5px;">编辑</a><a  title='+v.id +' href="#" style="margin-left:5px;" onclick="deleteiportol(\''+ v.id  +'\')">删除</a><a href="javascript:" style="margin-left:5px;" class="copylink">复制链接</a>' + 
			...
			_table.find("tr").find(".changeStatus").each(function(){
				$(this).click(function(){
					...
					$.ajax({
						type : "POST",
						url : "/web_mgt/w_pc_news_list/changeStatus",
						data : {
							"id" : _id
						},
						dataType: "json",
...
function deleteiportol(pid){
	bootbox.confirm("确定要删除吗?", function(result){
		if(result){
			$.ajax({
				type : "POST",
				url : "/web_mgt/w_pc_news_list/newsDelete",
				data : {
					"id" : pid
				},
				dataType: "json",
					...
					if (data != null && data != "") {
						if (data == 1) {
							bootbox.alert("新闻删除成功", function(result){
								...
								tableFun("/web_mgt/w_pc_news_list/listByFilter", pageData);
							});
						} else {
...
$("#addButton").click(function(){
	window.location.href = "../web_mgt/w_pc_news_list_add.html";
});
