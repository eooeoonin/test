...
/*  //查询项目文件
  $.ajax({
    type: "POST",
    url: "/project/p_repayment_edit/getLoanFileList/"+Request.loanCode,
  ...
  $(document).on('ready', function () {
    $.ajax({
      type : "POST",
      url : "/project/p_repayment_edit/showLoan/"+Request.loanCode,
      success : function(data) {
        if (data != null && data != "") {
          if (data.resCode == 0) {
            //获取数据
...
//                risk=data.data.risk,
                operdesc=data.data.disbureoperdesc,
                ...
                operdescId=data.data.disbureoperdescId;
...
/*    $("#fileinput").fileinput({
      language: 'zh', //设置语言
      uploadUrl: "/project/p_repayment_edit/uploadLoanFile/"+Request.loanCode, //上传的地址
...
//      maxImageHeight: 1000,//图片的最大高度
      ...
      $.ajax({
        type: "POST",
        url: "/project/p_repayment/getLoanFileList/"+Request.loanCode,
        success: function (data) {
          if (data != null && data != "") {
            tableBodyHtml = '';
            $.each(data.data, function (k, v) {
  ...
  $("#saveLoan").click(function () {
    ...
    $.ajax({
      type: "POST",
      url: "/project/p_repayment_edit/saveLoan",
      data: $("#loanForm").serialize(),
      success: function (data) {
        if (data != null && data != "") {
          if (data.resCode == 0) {
            bootbox.alert("操作成功", function () {
              window.location.href = "p_repayment_edit.html?loanCode="+ Request.loanCode ;
            });
          }
          else {
            bootbox.alert(data.msg);
  ...
  $("#killLoan").click(function () {
    $.ajax({
      type: "POST",
      url: "/project/p_repayment_edit/killLoan",
      data: $("#loanForm").serialize(),
      success: function (data) {
        if (data != null && data != "") {
          if (data.resCode == 0) {
            bootbox.alert("操作成功", function () {
              window.location.href = "p_repayment_list.html";
            });
          }
          else {
            bootbox.alert(data.msg);
...
/*function delFile(fileId){
  ...
  $.ajax({
    type: "POST",
    url: "/project/p_repayment_edit/deleteLoanFile/"+fileId,
    success: function (data) {
      if (data != null && data != "") {
        $("#"+fileId).removeAttr();
        $.ajax({
