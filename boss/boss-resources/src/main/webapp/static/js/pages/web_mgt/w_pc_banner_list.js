...
var _pages;
var _pageSize = 5;
var _banner_type = $('#available').val();
$(function () {
  /***
   *功能说明：表格相关操作
   *参数说明：
  ...
  var srhData = {"pageNo":"1","pageSize":_pageSize,"type":_banner_type};
  tableFun("/web_mgt/w_pc_banner_list/banner/page", srhData);	
  myPage();
  
  $('#available').change(function(){
	  _banner_type = $('#available').val();
	  srhData = {"pageNo":"1","pageSize":_pageSize,"type":_banner_type};
	  tableFun("/web_mgt/w_pc_banner_list/banner/page", srhData);	
	  myPage();
  });
  
//  $('#pcGreenSelected').click(function(){
//	  alert($('#available').val());
//  });
});
	...
	$.ajax({
		...
		success: function (data) {
			...
			$.each(_data, function (k, v) {
				...
				tableBodyHtml += '<td>' + (v.type == '12' ? 'PC 首页' : (v.type == '13' ? 'PC 新人' : 'error_type')) + '</td>';
				tableBodyHtml += '<td>' + dateStr + '</td>';
				tableBodyHtml += '<td class="w_available">' + (v.isShow == '1' ? '开启' : '关闭') + '</td>>';
				tableBodyHtml += '<td><a href="w_pc_banner_list_edit.html?id='+d_ID+'">编辑</a><a  title='+v.id +' href="#" style="margin-left:5px;" onclick="deleteiportol(\''+ v.id  +'\')">删除</a>' + 
			...
			_table.find("tr").find(".changeStatus").each(function(){
				$(this).click(function(){
					...
					$.ajax({
						type : "POST",
						url : "/web_mgt/w_pc_banner_list/banner/changeStatus",
						data : {
							"id" : _id,"type":_banner_type
						},
						dataType: "json",
						async:false,
						success : function(data) {
...
function orderDown(bannerDown) {
	$.ajax({
		type: "POST",
		url: "/web_mgt/w_pc_banner_list/banner/orderDown",  //下调
		dataType: "json",
		data: {"id": bannerDown.title,"type":_banner_type},
		success: function (data) {
			if (data == 0) {
				return;
			};
...
var myPage = function(){
	...
	$tcdPage.createPage({
		...
		backFn: function (p) {
			...
			tableFun("/web_mgt/w_pc_banner_list/banner/page", pageData);
		}
	});
}
...
function orderUp(bannerDown) {
	$.ajax({
		type: "POST",
		url: "/web_mgt/w_pc_banner_list/banner/orderUp",  //下调
		dataType: "json",
		data: {"id": bannerDown.title,"type":_banner_type},
		success: function (data) {
			if (data == 0) {
...
function deleteiportol(pid){
	bootbox.confirm("确定要删除吗?", function(result){
		if(result){
			$.ajax({
				type : "POST",
				url : "/web_mgt/w_pc_banner_list/banner/delete",
				data : {
					"id" : pid
				},
				dataType: "json",
