...
$(function () {
	...
	tableFun("/basedata_mgt/app_notice_list/notice/page", srhData);	
	myPage();
...
//          var d_availableTimeAfter = date.getFullYear() + '-' +( date.getMonth()+1) + '-' + date.getDate() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();  
        ...
        tableBodyHtml += '<td><a  href="app_notice_list_edit.html?id='+d_ID+'">编辑</a><a title='+v.id +' href="#" style="margin-left:5px;" onclick="deleteiportol(\''+ v.id  +'\')">删除</a>' + 
...
var changeStatusFun = function(_id){
	...
	$.ajax({
		type : "POST",
		url : "/basedata_mgt/app_notice_list/notice/changeStatus",
		data : {
			"id" : _id
		},
		dataType: "json",
...
function deleteiportol(pid){
	bootbox.confirm("确定要删除吗?", function(result){
		if(result){
			$.ajax({
				type : "POST",
				url : "/basedata_mgt/app_notice_list/notice/delete",
				data : {
					"id" : pid
				},
				dataType: "json",
					...
					if (data != null && data != "") {
						if (data == 1) {
							bootbox.alert("删除成功", function(result){
								...
								tableFun("/basedata_mgt/app_notice_list/notice/page", pageData);
							});
						} else {
							bootbox.alert("操作失败");
						}
