...
//    var ue = UE.getEditor('risk');
    ...
    $("#useRedPacket").on('ifUnchecked', function () {
        // $("#useRedPacket").iCheck('uncheck');
        //  alert($("#useRedPacket").val());
        $("#useRedPacket").val(0);
    ...
    $("#useRedPacket").on('ifChecked', function () {
        // alert($("#useRedPacket").prop("checked"));
        // alert($("#useRedPacket").val());
        $("#useRedPacket").val(1);
    ...
    $.ajax({
        type: "POST",
        url: "/project/p_wait_list/getContractList",
    ...
    $.ajax({
        type: "POST",
        url: "/project/p_wait_list/getLoanTypeList",
        success: function (data) {
            if (data != null && data != "") {
                tableBodyHtml = '';
    ...
    $.ajax({
        type: "POST",
        url: "/project/p_wait_list/getInvestGroupCode",
        success: function (data) {
            if (data != null && data != "") {
                if (data.resCode == 0) {
...
/*
    ...
    $.ajax({
        type: "POST",
        url: "/project/p_wait_list/getLoanFileList/" + Request.loanCode,
        success: function (data) {
            if (data != null && data != "") {
                tableBodyHtml = '';
                $.each(data.data, function (k, v) {
    ...
    $(document).on('ready', function () {
        $.ajax({
            type: "POST",
            url: "/project/p_wait_list/showLoan/" + Request.loanCode,
            success: function (data) {
                if (data != null && data != "") {
                    if (data.resCode == 0) {
                        //获取数据
...
/*        $("#fileinput").fileinput({
            language: 'zh', //设置语言
            uploadUrl: "/project/p_wait_list/uploadLoanFile/" + Request.loanCode, //上传的地址
...
//      maxImageHeight: 1000,//图片的最大高度
    ...
    $("#saveLoan").click(function () {
        ...
        var dara = $("#loanForm").serialize();
        dara += ("&useRedPacket=" + $("#useRedPacket").prop("checked") +
        ...
        $.ajax({
            type: "POST",
            url: "/project/p_wait_list/saveLoan",
            data: dara,
            success: function (data) {
                if (data != null && data != "") {
                    if (data.resCode == 0) {
                        bootbox.alert("操作成功", function () {
                            window.location.href = "p_wait_edit.html?loanCode=" + Request.loanCode;
                        });
                    }
                    else {
                        bootbox.alert(data.msg);
    ...
    $("#openLoan").click(function () {
        ...
        bootbox.confirm("确定要开标吗?", function (result) {
            if (result) {
                $("#interestStartDate").removeAttr('disabled'); // 去掉 disabled
                $.ajax({
                    type: "POST",
                    url: "/project/p_wait_list/openLoan/" + Request.loanCode,
                    ...
                    success: function (data) {
                        ...
                        $("#interestStartDate").attr('disabled','disabled'); // 添加 disabled
                    },
                    async: false
                });
            }
    ...
    $("#killLoan").click(function () {
        ...
        bootbox.confirm("确定要流标吗?", function (result) {
            if (result) {
                $.ajax({
                    type: "POST",
                    url: "/project/p_wait_list/killLoan",
                    data: $("#loanForm").serialize(),
                    success: function (data) {
                        if (data != null && data != "") {
                            if (data.resCode == 0) {
...
/*function delFile(fileId) {
    ...
    $.ajax({
        type: "POST",
        url: "/project/p_wait_list/deleteLoanFile/" + fileId,
        success: function (data) {
            if (data != null && data != "") {
                $("#" + fileId).removeAttr();
                $.ajax({
